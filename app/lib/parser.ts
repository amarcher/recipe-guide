...
function logUsage(
  ...
  sql`INSERT INTO api_usage (project, service, endpoint, tokens_in, tokens_out, model, metadata)
    VALUES ('recipe-guide', 'anthropic', ${endpoint}, ${tokensIn}, ${tokensOut}, ${MODEL}, ${JSON.stringify({ sourceUrl })})`.catch(
    (e) => console.error("[parser] usage log failed:", e)
  );
}
...
// URL → CookCard, respecting the ParsedRecipe cross-account cache. Used by
// both /api/parse and the Instagram sync path (when an IG caption links to
// a real recipe page).
export async function parseRecipeUrl(url: string): Promise<ParseResult> {
  const cached = await prisma.parsedRecipe.findUnique({ where: { sourceUrl: url } });
  if (cached) {
    return { card: cached.cardJson as unknown as CookCard, cached: true };
  }
...
Return only the JSON object.`;
  ...
  const raw = resp.content
    ...
    .map((b) => b.text)
    .join("\n");
  logUsage(resp.usage.input_tokens, resp.usage.output_tokens, url, "parse");
  ...
  await prisma.parsedRecipe.upsert({
    ...
    create: {
      sourceUrl: url,
      title: obj.title,
      cardJson: obj as unknown as object,
      modelUsed: MODEL,
    },
    update: {
      title: obj.title,
