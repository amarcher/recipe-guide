...
- **Storage cost**: 1024×1024 JPEG ≈ 250 KB. ~70 recipes today ≈ 18 MB Blob.
- **Deliberate gap**: existing `RecipeOverride` rows created BEFORE backfill don't include the URL — overrides are full-card replacements at fork/edit time. Those users keep seeing the canonical-or-vignette fallback chain via `card-resolver`. A separate one-shot to merge the URL into existing overrides can land later if it matters.
