...
A cheap, async, cross-session message bus for the agent team (planning scouts, the standing judge panel, the loop, the chair). Agents "DM" each other by **appending one line to the recipient's inbox file**; a recipient reads *its* unread by reading **one small file** and filtering `[ ]` lines. Same `[ ]` (unread) / `[x]` (read) idiom as `inbound-feedback.md`. No daemon, no API — just files. Survives restarts; cheap to poll.
...
```
.agents/bus/
  <agent>.inbox.md      # one inbox per agent; append-only; [ ] unread / [x] read
...
Read `.agents/bus/<me>.inbox.md`; for each `[ ]` line, act on it (open the payload if referenced), then flip `[ ]` → `[x]`. **Never delete** — `[x]` is the audit trail. Re-reading `[x]` lines is a no-op, so polling is safe and idempotent.

## Broadcast (one-to-many)

Append to `.agents/bus/topics/<topic>.md`. Subscribers tail the topic and track their own progress (their own `[ ]`/`[x]` copy, or a "last-seen line N" cursor in their inbox). Use a topic when many agents care (e.g. `planning-findings`); use a direct inbox when one does.
...
- **Idempotent by construction** — flipping `[ ]`→`[x]` and re-reads never double-act.
