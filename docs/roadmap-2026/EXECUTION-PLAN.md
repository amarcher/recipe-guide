...
The live task queue moved to **`docs/agent-loop/QUEUE.md`** — the loop's operational cursor (Open / Blocked / Backlog / Done). The autonomous-loop apparatus now lives under `docs/agent-loop/` (LOOP-OPS, JUDGE-PANEL, AGENT-BUS, judges-evolution-memo, inbound-feedback, QUEUE); the panel personas remain at `docs/roadmap-2026/positions/`. This file stays the roadmap rationale.
