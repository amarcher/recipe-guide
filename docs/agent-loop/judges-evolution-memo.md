...
- 2026-06-10 · rel-aggregate-tests · winner: candidate "honest vitest @/ config + .claude/worktrees exclude + manifest-drift guard, smallest body for most tests" (architect lens) · #47 · 1 builder DISQUALIFIED for committing an empty branch — only the branch-inspecting architect judge caught it (dossier-only judges scored it top); 4-way split chaired to winner; grafted *enforced* Prisma-in-vitest throwing-stub alias + WANT-spec tripwires for two found grocery bugs (range under-count, clove/cloves desync)
...
- 2026-06-10 · rel-aggregate-tests · aggregate.ts IS the grocery list — the (slug||item,unit) key it owns is the same one rebuildGroceryForPlan unions across committed meals, so a desync here makes the shared list lie; the range "8-10" collapsing to its low end and the clove/cloves non-merge are silent UNDER-buy bugs, and the right move is an it.skip WANT spec that becomes a tripwire when fixed, never blessing the bad total as correct. The vitest @/ alias that makes these real-module tests possible must genuinely keep @/app/lib/prisma unimportable.
...
- 2026-06-10 · rel-aggregate-tests · aggregate.ts is the (slug||item, unit) dedupe + sum-then-scale-once seam BOTH the execution mise and the planner grocery rollup ride on, so coverage must pin the real exported contract (not a reimpl) and ideally integration-test the buildMise→rebuildGroceryForPlan path; the shared vitest.config.ts is clean only if it aliases @ WITHOUT reopening the @/app/lib/prisma gotcha and excludes .claude/worktrees so sibling-agent test files don't leak — and the 8-10 range low-end collapse is a real grocery under-buy to fix later, not bless.
...
- 2026-06-10 · rel-aggregate-tests · for a no-UI feature, test legibility IS the product surface: the winning pattern is recipe-style fixture builders (ing/step/card) + course-grouped describe blocks + a found bug documented BOTH as a pinned-current assertion and an it.skip WANT spec. HARD LESSON: I scored a candidate top from its dossier prose — but it had committed NOTHING to its branch; only the architect's branch-inspection caught it. A glowing dossier is not delivered code — anchor at least a sanity glance to the actual diff/branch, never the summary alone.
...
- 2026-06-10 · rel-aggregate-tests · the (slug||item,unit) dedupe key is now a tested cross-boundary contract shared by execution mise and planner GroceryItem rollup — synonym slug-collapse (salt/kosher salt→one entry), distinct-unit separation, and null≈"" unit are the load-bearing invariants any gifting/shelf grocery-union feature will inherit; the global vitest.config.ts @/ alias is a latent Prisma-leak vector, so the winning convention scopes it (worktree exclude) and now enforces @/app/lib/prisma → throwing stub — future test authors cannot rely on the alias to pull Prisma into vitest.
...
- 2026-06-10 · rel-aggregate-tests · the only way to unit-test a @/-importing pure module is to alias @→root in vitest, which empirically reopens the documented vitest-no-prisma guard (proved @/app/lib/prisma loads fine under the alias) — so the fix that shipped is an explicit THROWING-STUB alias for @/app/lib/prisma (ordered before @, first-match-wins), NOT a comment claiming it "still fails to load" (two candidates shipped that false claim; I disproved both). Second: with .claude/worktrees present, a bare `vitest run` discovers sibling agents' tests (1168 across 102 files) unless the config excludes them — correctness, not scope creep. Third, vindicating the panel's whole premise: only branch-inspection caught a builder that committed NOTHING while reporting a full dossier — judge the artifact, not the story.
